...
def gsheet_csv_url(sheet_url: str, sheet_name: str = "Holdings") -> str:
    """Convert a Google Sheets share URL into a CSV export URL for a named tab.

    Uses the gviz endpoint which accepts tab names directly. This is more
    reliable than the export?gid=N approach since gids are random when sheets
    ...
    if not m:
        return sheet_url
    sheet_id = m.group(1)
    # URL-encode the sheet name to handle spaces / special chars
    from urllib.parse import quote
    safe_name = quote(sheet_name)
    return (
        f"https://docs.google.com/spreadsheets/d/{sheet_id}"
        f"/gviz/tq?tqx=out:csv&sheet={safe_name}"
    )
...
def load_holdings(source) -> pd.DataFrame:
    ...
    if _is_sheet_url(source):
        df = load_csv(gsheet_csv_url(source, sheet_name="Holdings"))
    else:
...
def load_realised(source) -> pd.DataFrame:
    try:
        if _is_sheet_url(source):
            df = load_csv(gsheet_csv_url(source, sheet_name="Realised"))
        else:
...
def load_watchlist(source) -> pd.DataFrame:
    """Load watchlist tab. Returns empty df if tab missing."""
    empty = pd.DataFrame(columns=["Stock Name", "Target Buy Price", "Notes", "Added By"])
    try:
        if _is_sheet_url(source):
            df = load_csv(gsheet_csv_url(source, sheet_name="Watchlist"))
        else:
...
def load_notes(source) -> pd.DataFrame:
    ...
    try:
        if _is_sheet_url(source):
            df = load_csv(gsheet_csv_url(source, sheet_name="Notes"))
        else:
